...
Security Note:
    ...
    Security modes:
    - When TINYIDP_MCP_ADMIN_SECRET is set, mutating operations require
      the admin_secret parameter to match.
    - When --readonly flag or TINYIDP_MCP_READONLY=true, mutating tools
...
def _check_admin_secret(tool_name: str, arguments: dict) -> Tuple[bool, str]:
    ...
    """
    required_secret = os.getenv("TINYIDP_MCP_ADMIN_SECRET")
    ...
    if not provided_secret:
        return False, f"TINYIDP_MCP_ADMIN_SECRET is set. Provide 'admin_secret' parameter for {tool_name}."
...
def _ensure_config() -> ConfigManager:
    ...
    global _config
    if _config is None:
        config_dir = os.getenv("TINYIDP_CONFIG_DIR", "./config")
...
async def list_tools() -> list[Tool]:
    ...
    return [
        ...
        Tool(
            name="list_users",
            description="List all configured users in TinyIDP",
            inputSchema={
                "type": "object",
                "properties": {},
                "required": [],
        ...
        Tool(
            name="create_user",
            description="Create a new user in TinyIDP",
            inputSchema={
                "type": "object",
                "properties": {
                    "username": {
        ...
        Tool(
            name="delete_user",
            description="Delete a user from TinyIDP",
            inputSchema={
                "type": "object",
                "properties": {
                    "username": {
        ...
        Tool(
            name="get_settings",
            description="Get current TinyIDP settings",
            inputSchema={
                "type": "object",
                "properties": {},
                "required": [],
        ...
        Tool(
            ...
            description="Update TinyIDP settings (issuer, audience, token expiry, etc.)",
            inputSchema={
                "type": "object",
                "properties": {
                    "issuer": {
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="TinyIDP MCP Server - Identity Provider tools for Claude Code",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Security modes:
  --readonly              Disable mutating tools (create, update, delete, generate)
  TINYIDP_MCP_READONLY    Same as --readonly (env var)
  TINYIDP_MCP_ADMIN_SECRET  Require admin_secret parameter for mutating tools

Examples:
  tinyidp-mcp                    # Full access
  tinyidp-mcp --readonly         # Read-only access
  TINYIDP_MCP_READONLY=true tinyidp-mcp  # Read-only via env var
        """
    )
    parser.add_argument(
        "--readonly",
    ...
    _readonly_mode = args.readonly or os.getenv("TINYIDP_MCP_READONLY", "").lower() in ("true", "1", "yes")
    ...
    if _readonly_mode:
        logger.info("Starting TinyIDP MCP Server in READONLY mode - mutating tools disabled")
    else:
        logger.info("Starting TinyIDP MCP Server...")
