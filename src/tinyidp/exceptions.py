"""
Typed exceptions for TinyIDP.
Provides clear, specific error handling across the application.
"""


class TinyIDPError(Exception):
    """Base exception for all TinyIDP errors."""

    def __init__(self, message: str, code: str = "TINYIDP_ERROR"):
...
class AuthenticationError(TinyIDPError):
...
# Client Errors
class ClientError(TinyIDPError):
...
class AuthCodeError(TinyIDPError):
...
class ConfigurationError(TinyIDPError):
