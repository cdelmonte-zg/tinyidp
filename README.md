...
```bash
git clone https://github.com/cdelmonte-zg/tinyidp.git
cd tinyidp
pip install .
```
...
python -m tinyidp init

# Or specify a custom path
python -m tinyidp init ./my-idp-config

# Then start with that config
python -m tinyidp --config ./my-idp-config
```
...
TINYIDP_CONFIG_DIR=/path/to/config python -m tinyidp

# With options
python -m tinyidp --port 8080 --debug
```
...
python -m tinyidp --debug
```

## MCP Server (Model Context Protocol)

TinyIDP includes an MCP server for integration with Claude Code and other MCP-compatible tools.
...
Or if TinyIDP is installed globally:
...
```json
{
  "mcpServers": {
    "tinyidp": {
      "command": "tinyidp-mcp",
      "env": {
        "TINYIDP_CONFIG_DIR": "/path/to/config"
      }
    }
  }
}
...
```json
{
  "mcpServers": {
    "tinyidp": {
      "command": "tinyidp-mcp",
      "env": {
        "TINYIDP_CONFIG_DIR": "/path/to/tinyidp/config"
      }
    }
  }
}
...
TinyIDP supports two security profiles:
...
TinyIDP supports key rotation with multiple keys in JWKS for seamless token validation during rotation:
